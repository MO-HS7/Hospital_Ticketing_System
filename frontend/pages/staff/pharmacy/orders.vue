  ...
  <div class="min-h-screen bg-[var(--color-bg-secondary)]">
    ...
    <header class="bg-[var(--color-bg-primary)] border-b border-[var(--color-border)] px-6 py-4">
      <div class="max-w-7xl mx-auto flex items-center justify-between">
        <div class="flex items-center gap-4">
          <h1 class="text-xl font-bold text-[var(--color-text-primary)]">{{ $t('orders.pharmacy') }}</h1>
          <span class="badge bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300">
            {{ $t('orders.title') }}
          </span>
        </div>
        <div class="flex items-center gap-4">
    ...
    <main class="max-w-7xl mx-auto px-6 py-8">
          ...
          <div class="flex items-start justify-between gap-4">
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-2">
                ...
                <span :class="statusClass(order.status)" class="badge text-xs">
            ...
            <button class="btn-outline text-sm px-3 py-1">{{ $t('common.view') }}</button>
          </div>
        </div>
      </div>
      ...
      <div v-if="pagination.lastPage > 1" class="flex justify-center gap-2 mt-6">
        <button :disabled="pagination.currentPage <= 1" class="btn-outline text-sm px-3 py-1" @click="fetchOrders(pagination.currentPage - 1)">
          {{ $t('common.previous') }}
        </button>
        <span class="text-sm text-[var(--color-text-muted)] px-3 py-1">{{ pagination.currentPage }} / {{ pagination.lastPage }}</span>
        <button :disabled="pagination.currentPage >= pagination.lastPage" class="btn-outline text-sm px-3 py-1" @click="fetchOrders(pagination.currentPage + 1)">
          {{ $t('common.next') }}
        </button>
      </div>
    </main>
    ...
    <div v-if="selectedOrder" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" @click.self="selectedOrder = null">
      <div class="bg-[var(--color-bg-primary)] rounded-2xl shadow-2xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-xl font-bold text-[var(--color-text-primary)]">{{ $t('orders.pharmacy') }}</h3>
        ...
        <div class="space-y-4 mb-6">
          ...
          <div>
            <label class="label">{{ $t('orders.items') }} (Medications)</label>
        ...
        <div v-if="canUpdate(selectedOrder)" class="border-t border-[var(--color-border)] pt-6 space-y-4">
          ...
          <div v-if="updateForm.status === 'completed'">
            <label class="label mb-2">{{ $t('orders.results') }} (Dispensed)</label>
            <textarea v-model="resultsText" class="input w-full" rows="4" placeholder="Dispensed medications and notes..."></textarea>
          </div>
          <div class="flex gap-3 justify-end">
            <button @click="selectedOrder = null" class="btn-outline px-4 py-2">{{ $t('common.cancel') }}</button>
            <button @click="submitUpdate" :disabled="updating || !updateForm.status" class="btn-primary px-4 py-2">
...
const formatResults = (results: any) => typeof results === 'string' ? results : JSON.stringify(results, null, 2)
const canUpdate = (order: any) => ['pending', 'processing'].includes(order.status)
...
const fetchOrders = async (page = 1) => {
  ...
  try {
    const res: any = await $fetch(`${config.public.apiBase}/orders`, {
      ...
      params: { type: 'pharmacy', status: activeStatus.value, page, per_page: 10 }
    })
    orders.value = res.data || []
    pagination.value = { currentPage: res.current_page || 1, lastPage: res.last_page || 1, total: res.total || 0 }
...
const submitUpdate = async () => {
  ...
  try {
    ...
    if (updateForm.value.status === 'completed' && resultsText.value) {
      try { body.results = JSON.parse(resultsText.value) }
      catch { body.results = { dispensed: resultsText.value } }
    }
  ...
  } catch (e: any) { console.error('Failed to update order:', e) }
