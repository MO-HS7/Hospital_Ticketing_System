      ...
      <div class="w-full max-w-md">
        <div class="card p-8 animate-fade-in">
          <div class="text-center mb-8">
            ...
            <p class="text-sm text-[var(--color-text-muted)] mt-2">{{ $t('landing.loginSubtitle') }}</p>
          </div>
          <div v-if="error" class="mb-4 p-3 rounded-lg bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 text-sm">{{ error }}</div>
          <form class="space-y-4" @submit.prevent="handleLogin">
            <div>
              <label for="email" class="block text-sm font-medium mb-1.5">{{ $t('auth.email') }}</label>
              <input id="email" v-model="form.email" type="email" class="input" :placeholder="$t('auth.emailPlaceholder')" required autocomplete="email" />
            </div>
            <div>
              <div class="flex items-center justify-between mb-1.5">
                <label for="password" class="text-sm font-medium">{{ $t('auth.password') }}</label>
                <NuxtLink to="/auth/forgot-password" class="text-xs text-primary-600 hover:underline">{{ $t('auth.forgotPassword') }}</NuxtLink>
              </div>
              <input id="password" v-model="form.password" type="password" class="input" :placeholder="$t('auth.passwordPlaceholder')" required autocomplete="current-password" />
            </div>
          ...
          <div class="relative my-6">
            <div class="absolute inset-0 flex items-center"><div class="w-full border-t border-[var(--color-border)]" /></div>
            <div class="relative flex justify-center text-sm"><span class="px-2 bg-[var(--color-bg-secondary)] text-[var(--color-text-muted)]">{{ $t('auth.noAccount') }}</span></div>
          </div>
          <NuxtLink to="/auth/register" class="btn-secondary w-full text-center">{{ $t('landing.createAccount') }}</NuxtLink>
...
const handleLogin = async () => {
  ...
  if (success) {
    ...
    redirect ? navigateTo(redirect) : redirectByRole()
  }
}
</script>
